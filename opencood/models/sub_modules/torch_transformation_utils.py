...
def project_cav_features_to_ego(x, record_len, pairwise_t_matrix, discrete_ratio=0.4, downsample_rate=1):
    ...
    pairwise_t_matrix = get_discretized_transformation_matrix(
        pairwise_t_matrix.reshape(-1, L, 4, 4), discrete_ratio, downsample_rate).reshape(B, L, L, 2, 3)
    ...
    for b in range(B):
        ...
        current_t_matrix = t_matrix[:, 0, :, :]
        current_t_matrix = get_transformation_matrix(current_t_matrix, (H, W))
        trans_x = warp_affine(batch_node_features[b], current_t_matrix, (H, W))
        out.append(trans_x)
