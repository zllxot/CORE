...
class VoxelNetIntermediateCORE(nn.Module):
    def __init__(self, args):
        super(VoxelNetIntermediateCORE, self).__init__()
        ...
        self.down_conv = Conv2d(128, 10, 1, 1, 0)
    ...
    def forward(self, data_dict):
        ...
        # compression layer
        if self.compression:
            vmfs_tsk, vmfs_rec = self.compression_layer(vmfs)
        else:
            vmfs_tsk = vmfs

        # information naive fusion
        vmfs_fusion = self.fusion_net(vmfs_tsk, record_len)
        ...
        if self.training and self.compression:
            ...
            output_dict['x_rec'] = self.down_conv(vmfs_rec)
