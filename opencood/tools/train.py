...
def main():
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        for i, batch_data in enumerate(train_loader):
            # the model will be eval mode during validation
            model.train()
            model.zero_grad()
            optimizer.zero_grad()
            ...
            # case1 : late fusion train --> only ego needed, and ego is random selected
            # case2 : early fusion train --> all data projected to ego
            # case3 : intermediate fusion --> ['ego']['processed_lidar']
            # becomes a list, which containing all data from other cavs as well
            if not opt.half:
        ...
        if epoch % hypes['train_params']['eval_freq'] == 0:
            ...
            # save the validate loss in a text file
            with open(saved_path + "/loss.txt", "a") as file:
                file.write('lr:' + str(param_group["lr"]) + ', epoch:' + str(epoch + 1) + ', validate loss:' + str(
                    valid_ave_loss) + "\n")
