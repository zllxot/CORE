...
class PointPillarRecLoss(nn.Module):
    def __init__(self, args):
        super(PointPillarRecLoss, self).__init__()
    ...
    def forward(self, output_dict, target_dict):
        ...
        if self.training:
            x_ideal = output_dict['x_ideal']
            x_rec = output_dict['x_rec']
    ...
    def rec_loss_func(self, input: torch.Tensor, target: torch.Tensor):
        l2_loss_list = []

        for i in range(input.shape[0]):
            l2_loss_list.append(torch.mean((target[i] - input[i]) ** 2))
    ...
    def logging(self, epoch, batch_id, batch_len, writer, pbar=None):
        ...
        if self.training:
            ...
            if pbar is None:
                print("[epoch %d][%d/%d], || Loss: %.4f || Conf Loss: %.4f || Loc Loss: %.4f || Rec Loss: %.4f" % (
                    epoch, batch_id + 1, batch_len, total_loss.item(), conf_loss.item(), reg_loss.item(),
                    rec_loss.item()))
            else:
                pbar.set_description("[epoch %d][%d/%d], || Loss: %.4f || Conf Loss: %.4f"
                                     " || Loc Loss: %.4f || Rec Loss: %.4f" % (
                                         epoch, batch_id + 1, batch_len,
                                         total_loss.item(), conf_loss.item(), reg_loss.item(), rec_loss.item()))

            writer.add_scalar('Regression_loss', reg_loss.item(),
                              epoch * batch_len + batch_id)
            writer.add_scalar('Confidence_loss', conf_loss.item(),
                              epoch * batch_len + batch_id)
            writer.add_scalar('Reconstruction_loss', rec_loss.item(),
                              epoch * batch_len + batch_id)
        else:
            if pbar is None:
                print("[epoch %d][%d/%d], || Loss: %.4f || Conf Loss: %.4f || Loc Loss: %.4f" % (
                    epoch, batch_id + 1, batch_len, total_loss.item(), conf_loss.item(), reg_loss.item()))
            else:
                pbar.set_description("[epoch %d][%d/%d], || Loss: %.4f || Conf Loss: %.4f"
                                     " || Loc Loss: %.4f" % (
